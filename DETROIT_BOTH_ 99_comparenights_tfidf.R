...
#save to file
write_csv(top_tfidf_per_speaker, "output/detroit_nightvsnight_tfidf_singleword.csv")
...
#save to file
write_csv(top_tfidf_per_speaker_bigrams, "output/detroit_nightvsnight_tfidf_bigrams.csv")




### NOW TRU-GRAMS (3 words) VERSION -------------------------------
...
# new bigram counts:
bigram_counts <- bigrams_filtered %>% 
  count(night, word1, word2, word3, sort = TRUE)
...
#pull top ones for each speaker into table
top_tfidf_per_speaker_trigrams <- speaker_bigrams_tfidf %>%
  group_by(night) %>%
  top_n(15) %>%
  slice(1:10) %>% #added to limit to 10 records per cand, even with ties 
  ungroup
...
#save to file
write_csv(top_tfidf_per_speaker_trigrams, "output/detroit_nightvsnight_tfidf_trigrams.csv")
